...
class KeyboardControl(object):
    ...
    def _parse_vehicle_keys(self, keys, milliseconds):
        ...
        steer_increment = 3e-4 * milliseconds  ### previous it is 3e-4
        if keys[K_LEFT] or keys[K_a]:
            self._steer_cache -= steer_increment
            #self._steer_cache = max(self._steer_cache, -0.5)
        elif keys[K_RIGHT] or keys[K_d]:
            self._steer_cache += steer_increment
            #self._steer_cache = min(self._steer_cache, 0.5)
        else:
            ##self._steer_cache = 0.0
            if self._steer_cache >= 0:
                self._steer_cache = max(0, self._steer_cache-3*steer_increment)
            else:
                self._steer_cache = min(self._steer_cache+3*steer_increment, 0)   
            
        #print('human_agent.py: self._steer_cache: ', self._steer_cache)
