...
RECORDER_INFO = [
    {
        'folder': "leaderboard/Scenario_Logs/HardBreakRoute",
        'start_time': 1, ### initial start frames looks weird, hence, we set the start frame to be 20 frames later.
        'duration': 0    ### if we set it to be 0, then it will use the maximum length
    },
...
def main():
    ...
    try:
        ...
        #### contains 'folder', 'start_time', and 'duration'
        #for recorder_info in RECORDER_INFO:
        path = 'leaderboard/Scenario_Logs/' 
        for recorder_info in os.listdir(path):    
            recorder_folder = path+recorder_info #recorder_info['folder'] ##
            recorder_start = 1 #recorder_info['start_time']
            recorder_duration = 0 #recorder_info['duration']

            # 0) Get the recorder files
            recorder_path_list = glob.glob(f"{os.getcwd()}/{recorder_folder}/*.log")
            if recorder_path_list:
                recorder_path = recorder_path_list[0]  ### a list of one element
            else:
            ...
            ##### read the recorded info (later it will be written to a .txt file)
            recorder_str = client.show_recorder_file_info(recorder_path, True)  ### https://carla.readthedocs.io/en/latest/ref_recorder_binary_file_format/
            ...
            #### every time the simulator does an update (advances the simulation one step), 
            #### it sends a message to the clients that are connected. We also call this update a "tick"
            for _ in range(100):
                print('world.tick() {} (to 100)'.format(_), end='\r')
                world.tick()

            # 3) Replay the recorder
            max_duration = float(recorder_str.split("\n")[-2].split(" ")[1])
            if recorder_duration == 0:
                recorder_duration = max_duration-0.5 - recorder_start
            ...
            with open(f"{recorder_path[:-4]}.txt", 'w') as fd:
                fd.write(recorder_str)  ### write the entire strings
            world.tick()
            ...
            # 5) Create the sensors, and save their data into a queue   ### first put the data in a cache then write to memory
            create_folders(endpoint, [[s[0], s[1].get('bp')] for s in SENSORS])  ## 'bp' is the blueprint defined in SENSORS
            blueprint_library = world.get_blueprint_library()
            sensor_queue = Queue()
            for sensor in SENSORS:

                # Extract the data from the sensor configuration
                sensor_id = sensor[0]
                attributes = sensor[1]
            ...
            while True:
                ...
                while True:
                    ...
                    try:
                        #print('calling sensor_queue.get')
                        sensor_data = sensor_queue.get(True)  ### block and 2.0 sec timeout and raises Empty
                        #print(sensor_data) [id, time, data] 
                        ### ('IMUTest', 266533, <carla.libcarla.IMUMeasurement object at 0x7f64ec437330>)
                    ...
                    data = sensor_data[2]
                    if imu_logs:
                        if int(FPS*recorder_start + frame_diff) < len(imu_logs):
                            imu_data = imu_logs[int(FPS*recorder_start + frame_diff)]
                        else:
                            print('The index goes out of the length. Frame_diff: ', frame_diff) 
                            imu_data = [[0,0,0], [0,0,0]]   
                    else:
                        imu_data = [[0,0,0], [0,0,0]]
                    ##### parallel saving one frame of all sensors
